...
// Approximate exchange rates (local currency per 1 USD)
const FX = {
...
const NPR_TO_USD = FX.NPR;

function toLocal(amountNPR, targetCurrency, multiplier) {
  const usd = amountNPR / NPR_TO_USD;
  const adjustedUSD = usd * multiplier;
  const rate = FX[targetCurrency];
  if (!rate) {
...
for (const [code, config] of Object.entries(multipliers)) {
  ...
  for (const [category, entries] of Object.entries(np.jobs)) {
    jobs[category] = entries.map((job) => ({
      ...job,
      salaryMin: toLocal(job.salaryMin, currency, multiplier),
      salaryMax: toLocal(job.salaryMax, currency, multiplier),
    }));
  }
...
console.log(`✓ Regenerated ${count} countries with proper currency conversion`);
